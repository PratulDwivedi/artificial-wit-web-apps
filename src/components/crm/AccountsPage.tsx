...
import { Loader2, ArrowLeft, RefreshCw, Globe, Phone, Mail } from 'lucide-react'
...
const FN_GET_ACCOUNTS = 'crm.fn_get_accounts'
...
export function AccountsPage() {
  ...
  const loadList = useCallback(async (silent = false) => {
    ...
    HttpHelper.rpcInvalidate(FN_GET_ACCOUNTS)
    const { data: res, error } = await HttpHelper.rpc<AccountRow[]>(FN_GET_ACCOUNTS)
    if (error || !res?.is_success) toast.error(error ?? res?.message ?? 'Failed to load accounts')
  ...
  const openDetail = async (id: number) => {
    ...
    const { data: res, error } = await HttpHelper.rpc<Account360[]>(FN_GET_360, { p_account_id: id })
    setDetailLoading(false)
    if (error || !res?.is_success) { toast.error(error ?? res?.message ?? 'Failed to load account'); return }
    if (res.data?.[0]) setDetail(res.data[0])
  }
  ...
  if (detail) {
    ...
    return (
      <div className="flex-1 flex flex-col overflow-auto" style={{ background: 'var(--c-base)' }}>
        {/* Header */}
        <div className="flex items-center gap-3.5 flex-wrap px-5 pt-4 pb-3">
          <button onClick={() => setDetail(null)} className="p-2 rounded-lg border"
            style={{ background: 'var(--c-panel)', borderColor: 'var(--c-border-strong)', color: 'var(--c-t3)' }}>
            <ArrowLeft size={15} />
          </button>
          <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white font-bold text-[15px]" style={{ background: 'var(--c-primary)' }}>
            {initials(a.name)}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-[16px] font-semibold" style={{ color: 'var(--c-t1)' }}>{a.name}</h1>
              {a.account_type && <span className="text-[11px] font-medium rounded-full px-2.5 py-0.5 capitalize" style={typePill(a.account_type)}>{a.account_type}</span>}
            </div>
            <div className="text-[12px] mt-0.5 flex items-center gap-3 flex-wrap" style={{ color: 'var(--c-t4)' }}>
        ...
        <div className="flex gap-3 px-5 flex-wrap">
          {[
            ...
            <div key={x.l} className="rounded-xl border px-4 py-2.5 min-w-[150px]" style={{ background: 'var(--c-panel)', borderColor: 'var(--c-border)' }}>
              <div className="text-[11px] uppercase tracking-wide" style={{ color: 'var(--c-t4)' }}>{x.l}</div>
              <div className="text-[17px] font-semibold mt-0.5" style={{ color: x.pos ? '#16a34a' : 'var(--c-t1)' }}>{x.v}</div>
              <div className="text-[11px] mt-0.5" style={{ color: 'var(--c-t4)' }}>{x.d}</div>
            </div>
          ))}
        </div>

        {/* Columns */}
        <div className="flex-1 flex gap-4 px-5 py-4 items-start flex-wrap lg:flex-nowrap">
          {/* Contacts */}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: 'var(--c-base)' }}>
      <div className="flex items-center gap-3 px-5 py-3 border-b" style={{ background: 'var(--c-topbar)', borderColor: 'var(--c-border)' }}>
        <h1 className="text-[15px] font-semibold" style={{ color: 'var(--c-t1)' }}>
          Accounts <span className="text-[12px] font-normal" style={{ color: 'var(--c-t4)' }}>{accounts?.length ?? 0} total</span>
        </h1>
        <div className="flex-1" />
        {detailLoading && <Loader2 size={15} className="animate-spin" style={{ color: 'var(--c-t4)' }} />}
        <button onClick={() => loadList(true)} className="p-2 rounded-lg border" title="Refresh"
          style={{ background: 'var(--c-panel)', borderColor: 'var(--c-border-strong)', color: 'var(--c-t3)' }}>
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto px-5 py-4">
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--c-panel)', borderColor: 'var(--c-border)' }}>
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {['Account', 'Industry', 'Type', 'Contacts', 'Open deals', 'Open value', 'Owner'].map(h => (
                  <th key={h} className="text-left text-[11px] uppercase tracking-wide px-3.5 py-2.5 border-b"
                    style={{ color: 'var(--c-t4)', borderColor: 'var(--c-border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(accounts ?? []).map(a => (
                <tr key={a.id} onClick={() => openDetail(a.id)} className="cursor-pointer hover:opacity-90">
                  <td className="px-3.5 py-2.5 border-b" style={{ borderColor: 'var(--c-border)' }}>
                    <div className="flex items-center gap-2.5">
                      <span className="w-[28px] h-[28px] rounded-lg flex items-center justify-center text-[10px] font-bold text-white flex-none" style={{ background: 'var(--c-primary)' }}>
                        {initials(a.name)}
                      </span>
                      <div>
                        <div className="text-[13px] font-semibold" style={{ color: 'var(--c-t1)' }}>{a.name}</div>
                        {a.website && <div className="text-[11px]" style={{ color: 'var(--c-t4)' }}>{a.website.replace(/^https?:\/\/(www\.)?/, '')}</div>}
                      </div>
                    </div>
                  </td>
                  <td className="px-3.5 py-2.5 border-b text-[13px]" style={{ borderColor: 'var(--c-border)', color: 'var(--c-t2)' }}>{a.industry ?? '—'}</td>
                  <td className="px-3.5 py-2.5 border-b" style={{ borderColor: 'var(--c-border)' }}>
                    {a.account_type ? <span className="text-[11px] font-medium rounded-full px-2.5 py-0.5 capitalize" style={typePill(a.account_type)}>{a.account_type}</span> : '—'}
                  </td>
                  <td className="px-3.5 py-2.5 border-b text-[13px]" style={{ borderColor: 'var(--c-border)', color: 'var(--c-t2)' }}>{a.contact_count}</td>
                  <td className="px-3.5 py-2.5 border-b text-[13px]" style={{ borderColor: 'var(--c-border)', color: 'var(--c-t2)' }}>{a.open_deal_count}</td>
                  <td className="px-3.5 py-2.5 border-b text-[13px] font-medium" style={{ borderColor: 'var(--c-border)', color: 'var(--c-t1)' }}>{fmtINR(a.open_deal_value)}</td>
                  <td className="px-3.5 py-2.5 border-b text-[12px]" style={{ borderColor: 'var(--c-border)', color: 'var(--c-t3)' }}>{a.owner_name ?? '—'}</td>
                </tr>
              ))}
              {(accounts ?? []).length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-[13px]" style={{ color: 'var(--c-t4)' }}>No accounts yet</td></tr>
              )}
            </tbody>
          </table>
        </div>

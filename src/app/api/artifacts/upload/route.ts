...
export async function POST(request: NextRequest) {
  ...
  let res: Response
  try {
    res = await fetch(`${apiBase}/file/upload/artifacts?content_type=application/octet-stream`, {
      method:  'POST',
  ...
  if (!res.ok) {
    return NextResponse.json({ success: false, error: `Upload failed with HTTP ${res.status}`, detail: raw }, { status: 502 })
  }
